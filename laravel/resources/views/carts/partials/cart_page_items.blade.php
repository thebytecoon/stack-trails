                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-2 flex items-center space-x-4">
                        <div class="flex items-center border border-gray-300 rounded-md relative">
                            <button
                                hx-post="{{ route('carts.sub', [$cart->product_id]) }}"
                                hx-target="#cart-page-items"
                                hx-swap="outerHTML"
                                hx-vals='{"display": "page"}'
                                hx-indicator="#loading-spinner-page-{{ $loop->index }}"
                            ...
                            <button 
                                hx-post="{{ route('carts.add', [$cart->product_id]) }}"
                                hx-target="#cart-page-items"
                                hx-swap="outerHTML"
                                hx-vals='{"display": "page"}'
                                hx-indicator="#loading-spinner-page-{{ $loop->index }}"
                            ...
                            <img src="{% static 'spiner.gif' %}" alt="spiner" class="absolute top-1/2 left-12 transform -translate-x-1/2 -translate-y-1/2 htmx-indicator" style="max-width: 20px; max-height: 20px;" id="loading-spinner-page-{{ $loop->index }}">
                        </div>
                        <button 
                            hx-post="{{ route('carts.destroy', [$cart->product_id]) }}"
                            hx-target="#cart-page-items"
                            hx-swap="outerHTML"
                            hx-vals='{"display": "page"}'
